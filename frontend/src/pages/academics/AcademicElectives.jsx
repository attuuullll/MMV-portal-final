...
const AcademicElectives = () => {
  ...
  useEffect(() => {
    ...
    const fetchData = async () => {
      setLoading(true);
      try {
        const response = await fetch(`${API_BASE_URL}/academics/electives/${activeCategory}`, {
          headers: { 'Authorization': `Bearer ${localStorage.getItem('token')}` }
        });
        if (response.ok) setData(await response.json());
      } catch (error) { console.error("Failed to fetch electives data", error); }
  ...
  if (!catSlug) {
    return (
      <div className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-12">
        ...
        <div><h1 className="text-3xl font-bold text-primary">Electives</h1></div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {categories.map((cat) => (
            <button key={cat} onClick={() => navigate(`/academics/electives/${toSlug(cat)}`)}
  ...
  return (
    <div className="max-w-6xl mx-auto space-y-8 animate-fade-in pb-12">
      <button onClick={() => navigate('/academics/electives')}
        className="flex items-center text-sm font-medium text-primary hover:text-secondary transition-colors">
        ← Back to Electives
      </button>
      <div><h1 className="text-3xl font-bold text-primary">Electives — {activeCategory}</h1></div>
      ...
      <div className="bg-blue-50 rounded-2xl p-8 border-2 border-secondary min-h-[300px]">
        {loading ? (
          ...
          <div>
            {data.length === 0 ? (
              <div className="text-center py-12"><p className="text-muted">No elective documents uploaded for {activeCategory} yet.</p></div>
            ) : (
...
export default AcademicElectives;
